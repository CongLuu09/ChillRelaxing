package com.example.chillrelaxing.ui.Player.AirTravel;
...
public class AirTravelActivity extends AppCompatActivity {
            ...
            if (!layers.isEmpty()) {
                ...
                new Thread(() -> {
                    ...
                    for (LayerSound layer : layers) {
                        db.insertSound(layer.getName(), layer.getIconResId(), layer.getSoundResId(), "air");
    ...
    private void loadSavedSoundsFromDb() {
        new Thread(() -> {
            AppDatabase db = new AppDatabase(this);
            List<LayerSound> savedLayers = db.getAllSavedSounds("air");
